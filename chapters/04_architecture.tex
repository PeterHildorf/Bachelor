...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{figures/mvvmarkitektur.png}
...
\textbf{ViewModel-laget} indeholder applikationens logik og fungerer som bindeled mellem View og netværkslaget:
...
\begin{figure}[H]
  \centering
  % \includegraphics[width=\textwidth]{figures/sequence_image}
  \fbox{\parbox{0.9\textwidth}{\centering\vspace{2cm}
    \textit{[Figur: Sekvensdiagram — billedflow]}\\
    UIImage → jpegData(0.8) → multipart POST /api/v1/extract\\
    → Vision OCR (DOCUMENT\_TEXT\_DETECTION)\\
    → averageSymbolConfidence()\\
    → [confidence < 0.60 → FlaggedForReview]\\
    → Claude FormatEntities() → ExtractionResult JSON\\
    → Swift decode → EntitiesBubble
  \vspace{2cm}}}
