...
\textbf{Lavere hallucinationsrate.} Anthropics Constitutional AI-metode~\cite{anthropic_claude} er designet til at minimere situationer, hvor modellen opfinder information, der ikke er til stede i inputtet, hvilket er et kritisk krav i en vedligeholdelseskontekst, hvor forkerte instruktioner kan medføre sikkerhedsrisici.

\textbf{Struktureret JSON-output.} Claude Haiku følger stringent strukturerede promptinstruktioner og returnerer konsekvent validt JSON, hvilket forenkler parsing-laget i Go-middlewaren og reducerer behovet for fejlhåndtering.
...
# Cross-kompiler til Linux
GOOS=linux GOARCH=amd64 go build -o arkyn-server ./cmd/server/

# Stop service, kopier binær, genstart
